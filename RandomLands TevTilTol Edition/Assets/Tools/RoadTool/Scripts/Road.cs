...
public class Road : MonoBehaviour 
{
	...
	public void Refresh()
	{
		...
		for (int i = 0; i < v.Count; i++) {
			...
			v [i] = v [i] + new Vector3 (0, points [Mathf.Clamp ((int)Mathf.Round (((float)i + 0.001f) / 4f), 0, (points.Count - 1))].y, 0) + new Vector3(0f, groundOffset, 0f);
			//print ( (int)Mathf.Round((float)i/4f));
			//Gizmos.DrawSphere (new Vector3 (0f, points [(int)Mathf.Floor (i / 4)].y, 0f), 0.2f);
			v [i] -= diff;
		}
		...
		if (col == null)
			col = this.gameObject.AddComponent<MeshCollider>() as MeshCollider;
		col.sharedMesh = m;
	}
